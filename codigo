...
if not df.empty:
    ...
    # Cria colunas vazias para evitar erros de KeyError
    df["Mês/Ano"] = None
...
with tab_lancamentos:
    ...
    if st.session_state.dados.empty:
        st.info("Nenhum dado registrado ainda. Use a barra lateral para começar!")
    else:
        col_f1, col_f2, col_f3 = st.columns(3)
        with col_f1: filtro_tipo = st.multiselect("Tipo", ["Entrada", "Saída"], default=["Entrada", "Saída"])
        with col_f2: 
            lista_cats = sorted(list(set(st.session_state.dados["Categoria"])))
            filtro_cat = st.multiselect("Categoria", lista_cats, default=lista_cats)
        with col_f3:
            data_min = st.session_state.dados["Data"].min().date()
            data_max = st.session_state.dados["Data"].max().date()
            periodo = st.date_input("Período", value=(data_min, data_max))
        
        df_filtrado = st.session_state.dados[(st.session_state.dados["Tipo"].isin(filtro_tipo)) & (st.session_state.dados["Categoria"].isin(filtro_cat))]
        if len(periodo) == 2:
...
with tab_resumo:
    ...
    if st.session_state.dados.empty:
        ...
        total_entradas = st.session_state.dados[st.session_state.dados["Tipo"] == "Entrada"]["Valor"].sum()
        total_saidas = st.session_state.dados[st.session_state.dados["Tipo"] == "Saída"]["Valor"].sum()
...
with tab_orcamento:
    ...
    with col_met2:
        ...
        if not st.session_state.dados.empty and "Mês/Ano" in df.columns:
            gastos_mes = df[(df["Tipo"] == "Saída") & (df["Mês/Ano"] == mes_atual)].groupby("Categoria")["Valor"].sum().reset_index()
        else:
            gastos_mes = pd.DataFrame(columns=["Categoria", "Valor"])
...
with tab_graficos:
    ...
    if st.session_state.dados.empty:
        ...
        with col_g2:
            df_saidas = st.session_state.dados[st.session_state.dados["Tipo"] == "Saída"]
            if not df_saidas.empty:
                fig_pie = px.pie(df_saidas, values="Valor", names="Categoria", hole=0.4)
                st.plotly_chart(fig_pie, use_container_width=True)
        
        df_evolucao = st.session_state.dados.sort_values("Data")
